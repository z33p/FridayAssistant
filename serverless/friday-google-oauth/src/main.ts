...
import { RequestData } from './contracts/RequestData';

const SCOPES: string[] = ['https://www.googleapis.com/auth/gmail.send'];

async function getAccessToken(client: OAuth2Client, data: RequestData): Promise<LambdaResponse> {
...
async function generateOAuthUrl(client: OAuth2Client, _: RequestData): Promise<LambdaResponse> {
...
async function main(request_data: RequestData): Promise<LambdaResponse> {
  ...
  const lambdaActions: Record<string, (client: OAuth2Client, data: RequestData) => Promise<LambdaResponse>> = {
    "GENERATE_OAUTH_URL": generateOAuthUrl,
    "GET_ACCESS_TOKEN": getAccessToken
  };

  const response = await lambdaActions[request_data.action](client, request_data);
  return response;
}

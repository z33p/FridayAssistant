...
currentTag=$(grep -oP 'image: z33p/friday-todo-manager:\K[0-9]{4}-[0-9]{2}-[0-9]{2}.v[0-9]+' "$DEPLOYMENT_FILE")
...
sed -i "s|image: z33p/friday-todo-manager:.*|image: z33p/friday-todo-manager:$dockerTag|g" "$DEPLOYMENT_FILE"
...
# Tag para Docker Hub
docker tag friday-todo-manager:$dockerTag docker.io/z33p/friday-todo-manager:$dockerTag

# Push para Docker Hub
docker image push docker.io/z33p/friday-todo-manager:$dockerTag 

# Aplica service
kubectl apply -f k8s/friday-todo-manager-service.yml
